...
function openTicketModal(e) {
  ...
  tickettextDiv.addEventListener("keypress", handelKeyPress);
...
function handelKeyPress(e) {
  ...
  if (e.key == "Enter" && isTextTyped && e.target.textContent) {
    ...
    let filterSelected = document.querySelector(".selected-filter")
      .classList[1]; // To get color class we use [1]
    let ticketId = uuid();
...
// Edit feature
let ticketfilter = document.getElementById("color");
let ticketvalue = document.getElementById("value");
let ticketid = document.getElementById("id");
...
function editopenModal() {
  ...
  tickettextDiv.addEventListener("keypress", (e) => {
    if (e.key == "Enter" && isTextTyped && e.target.textContent) {
      //console.log("inside if");
      console.log(document.querySelector(".selected-filter"));
      let filterSelected = document.querySelector(".selected-filter")
        .classList[1]; // To get color class we use [1]
        console.log(ticketfilter);
      let ticketId = uuid();
      // if(filterSelected==ticketfilter){
      //   filterSelected=ticketfilter
      //   }
      let ticketInfoObject = {
...
// function handelKeyPressEdit(e){
//   console.log(e);
//   if (e.key == "Enter" && isTextTyped && e.target.textContent) {
//     //console.log("inside if");
//     let filterSelected =
//       document.querySelector(".selected-filter").classList[1];  // To get color class we use [1]
//     let ticketId = uuid();
//     let ticketInfoObject = {
//       ticketFilter: filterSelected,
//          ticketValue: e.target.textContent,
...
function appendTicket(ticketInfoObject) {
           ...
           <div class="ticket-header ${ticketFilter}"></div>
              <div class="ticket-content">
                  <div class="ticket-info">
                      ...
                      <div class="ticket-delete cursor"><i class="fa-solid fa-trash"></i></div>
                      <div class="ticket-edit cursor">
  ...
  let deleteTicketsBtn = ticketDiv.querySelector(".ticket-delete");

  deleteTicketsBtn.addEventListener("click", function (e) {
    ticketDiv.remove();
    let allTickets = JSON.parse(myDB.getItem("allTickets"));
  ...
  let id = ticketId;
  //console.log(id);
  EditTicketBtn.addEventListener("click", function (sel) {
    console.log(id);
    let data = localStorage.getItem("allTickets");
    data = JSON.parse(data);
    console.log(data);
    //console.log(data[0].ticketId);

    for (let i = 0; i < data.length; i++) {
      //console.log(data[i].ticketId);

      if (data[i].ticketId === id) {
        console.log(i);
        ticketfilter = data[i].ticketFilter;
        ticketid = data[i].ticketId;
        ticketvalue = data[i].ticketValue;

        deleteTicketsBtn.click();
        editopenModal();
      }
    }
  });
